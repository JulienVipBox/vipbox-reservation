...
- Décider du sort de `postToCrm()` (câblage dans `payment-handler.ts`, mapping déjà prêt) — **confirmé avec Julien (2026-09-03) : les commandes tunnel ne remontent aujourd'hui dans le CRM d'aucune façon** (`crm_prestation_id` reste `null` sur toutes les réservations, `postToCrm()` n'est appelé nulle part). Une commande tunnel réellement payée est donc invisible pour le CRM/logistique utilisé par le reste de l'entreprise (autres canaux : vipboxbooking.com, photoshaker.com) tant que ce n'est pas câblé — actuellement différé à la demande explicite de Julien, mais à garder en tête comme un vrai manque opérationnel avant d'ouvrir le tunnel à de vrais clients, pas juste un nice-to-have.
